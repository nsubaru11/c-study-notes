...
int indexOf(const BinarySearchTree* tree, const int value) {
	if (tree->label > value) {
		return tree->left ? tree->indexOf(tree->left, value) : ~0;
	} else if (tree->label < value) {
		int leftSize = tree->left ? tree->left->size : 0;
		if (tree->right) {
...
BinarySearchTree* higher(BinarySearchTree* tree, const int value) { return findBoundary(tree, value, false, true); }

BinarySearchTree* lower(BinarySearchTree* tree, const int value) { return findBoundary(tree, value, false, false); }

BinarySearchTree* ceiling(BinarySearchTree* tree, const int value) { return findBoundary(tree, value, true, true); }

BinarySearchTree* flooring(BinarySearchTree* tree, const int value) { return findBoundary(tree, value, true, false); }

int count(const BinarySearchTree* tree, const int value) {
	if (tree->label > value) {
		return tree->left ? tree->count(tree->left, value) : 0;
	} else if (tree->label < value) {
		return tree->right ? tree->count(tree->right, value) : 0;
	} else {
		return tree->cnt;
	}
}
